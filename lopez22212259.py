...
den = [C*L,C*R,1]
sys = ctrl.tf(num,den)
print(f"Funciòn de transferencia del sistema: {sys} ")

# Componentes del controlador
kP,kI,kD = 289.1145,7034.8743,0.39135
Cr = 1E-6
Re = 1/(kI*Cr)
Rr = kP*Re
Ce = kD/Rr
...
print (f"La funciòn de transferencia del controlador PID es {PID}")

# Sistema de control en lazo cerrado
X = ctrl.series(PID,sys)
sysPID = ctrl.feedback(X,1,sign = -1)
print (f"Funciòn de transferencia del sistema de control de lazo cerrado {sysPID}")

# Respuesta del sistema en lazo abierto y en lazo cerrado
clr1 = np.array([145,18,188])/255
clr2 = np.array([0,128,157])/255
